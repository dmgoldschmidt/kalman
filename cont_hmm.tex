...
Then using \eqref{mu:1}, we get

\begin{align*}
\mu^t\Sigma^{-1}\mu &= (\mu_1^t(\Sigma_1+\Sigma_2)^{-1}\Sigma_2 +\mu_2^t(\Sigma_1+\Sigma_2)^{-1}\Sigma_1)(\Sigma_1^{-1}\mu_1+\Sigma_2^{-1}\mu_2) \\
&= 2\mu_1^t(\Sigma_1+\Sigma_2)^{-1}\mu_2,
\end{align*}

whence \eqref{C} becomes
\begin{equation} \label{C:1}
  \begin{split}
  C &= \mu_1^t\Sigma_1^{-1}\mu_1 +\mu_2^t\Sigma_2^{-1}\mu_2 - 2\mu_1^t(\Sigma_1+\Sigma_2)^{-1}\mu_2\\
  &=
  \end{split}
\end{equation}

 \beta_t(s) &= \frac{P_{t+1,T}}{\sigma_{b,t}\sqrt{2\pi}}\exp\left(-\frac{(s-\mu_{b,t})^2}{2\sigma_{b,t}^2},\right),\label{beta:1}
\end{align}


%\gamma_t(s) :&= \alpha_t(s)\beta_t(s) = Pr(x_1,x_2,\dots,x_T ~ \text{\& state $s$ at time $t$}\mid \theta).
$$
Then for any time $t$, we have
\begin{equation*}
Pr(\text{state $s$ at time $t$}\mid x_1,x_2,\dots x_T,\theta) = \frac{\gamma_t(s)}{P_{1,t}},\quad\text{and}\quad
P_{1,T} = \int_{-\infty}^{\infty}\gamma_t(s)ds.
\end{equation*}


  &= \frac{\sigma_x^2}{a^2\sigma_{a,t-1}^2}\mu_{a,t-1} + x_t \\
  &=\frac{\sigma_x^2}{(\rho^2+1)\sigma_{a,t-1}^2}\mu_{a,t-1} + x_t \\
  &=\frac{\sigma_x^2\sigma_{a,t-1}^2}{(\sigma_0^2+\sigma_{a,t-1}^2)\sigma_{a,t-1}^2}\mu_{a,t-1} + x_t \\
  &=\frac{\sigma_x^2}{(\sigma_0^2+\sigma_{a,t-1}^2)}\mu_{a,t-1} + x_t ,


&= \frac{P_{1,t-1}\sigma_0}{\sqrt{2\pi}\sigma_x\sigma_{a,t-1}a}\exp-\frac{1}{2}
    \left(\frac{a_1^2(a_1s - b_1a_1^{-1})^2 +\rho_1^2(x_t-\mu_{a,t-1})^2}{a_1^2\sigma_x^2}\right) \\


& =\frac{v}{2\pi}\exp\left\{\frac{t^2}{2}\left((1 - \frac{1}{a^2}\right)\right\}\int_0^{\infty}s\alpha_{n-1}(s)
\exp\left\{-\frac{1}{2}(as - ta^{-1})^2\right\}ds,
\end{align*}
where $a^2 := (x_{n-1}-\mu)^2+1$.  Then the substitution $u := as-ta^{-1}$ yields
\begin{equation}\label{alpha:1}
\alpha_n(t) = \frac{v}{2a^2\pi}\exp\left\{\frac{t^2}{2}\left((1 - \frac{1}{a^2}\right)\right\}
\int_{-\infty}^{\infty}|u+ta^{-1}|\alpha_{n-1}(a^{-1}u+ta^{-2})\exp\left\{-\frac{1}{2}u^2\right\}du.
\end{equation}

\int_0^{\infty}\exp\left{-\frac{1}{2}(a^2x^2 + 2bx + c)\right}dx 
&= \exp\left{\frac{1}}{2}(b^2a^{-2}-c)\right}\int_0^{\infty}\exp\left{-\frac{1}{2}(ax+ba^{-1})^2}dx \\
&= a^{-1}\exp\left{\frac{1}}{2}(b^2a^{-2}-c)\right}\int_{ba^{-1}}^{\infty}\exp{-\frac{1}{2}u^2}du\\
&= \frac{\sqrt{\pi}}{2a}e^{b^2a^{-2}-c}\text{erfc}(ba^{-1}).
\end{align*}

Then for \eqref{alpha} we have
\begin{equation}
  \begin{split}
...
\xi_n := \frac {x_n-\mu}{v}.
$$
If we put $\hat{x}_n := \sqrt{\xi_n^2+1}$, we can complete the square to obtain
\begin{align*}
\alpha_{n+1}(t) &= 
\frac{v}{2\pi}\int_0^\infty s\alpha_n(s)\exp\left\{-\frac{v^2}{2}\left[(\hat{x}_ns- \frac{t}{\hat{x}_n})^2 + t^2(1-\frac{1}{\hat{x}_n^2}\right]\right\}ds,\\
\end{align*}

Similarly, we get
$$
\beta_{n-1}(t) = \int_0^{\infty}\beta_n(s)B(x_n | s))Pr(t | s)ds.
$$
\end{document}


      Put $\rho := \sigma_1/\sigma_2$, then
    $$
    \frac{\rho^2}{1+\rho^2} = \frac{\sigma_1^2}{\sigma_1^2+\sigma_2^2},\quad \text{and}
    $$
    \begin{equation}\label{Q(x):1}
      \begin{split}
  Q(x) &= \frac{(x-\mu_1)^2 + \rho^2(x-\mu_2)^2}{2\sigma_1^2}\\
  &= \frac{(1+\rho^2)x^2 - 2(\mu_1+\rho^2\mu_2)x + \mu_1^2+\rho^2\mu_2^2}{2\sigma_1^2} \\
  &=: \frac{a^2x^2-2bx+c}{2\sigma_1^2}\\
  &= \frac{(ax-a^{-1}b)^2 + c-a^{-2}b^2}{2\sigma_1^2}, \\
    \end{split}
\end{equation}
where
$$
  a^2 = 1+\rho^2,\quad b = \mu_1 + \rho^2\mu_2,\quad\text{and}\quad c = \mu_1^2+\rho^2\mu_2^2.
$$
  Then
  \begin{equation}\label{Q(x):2}
    \begin{split}
    c-a^{-2}b^2 &= \mu_1^2+\rho^2\mu_2^2 - \frac{\mu_1^2+2\rho^2\mu_1\mu_2 + \rho^4\mu_2^2}{1+\rho^2} \\
    &= \frac{\mu_1^2+\rho^2\mu_2^2 + \rho^2\mu_1^2 + \rho^4\mu_2^2 - \mu_1^2-2\rho^2\mu_1\mu_2 - \rho^4\mu_2^2}{1+\rho^2}\\
    &= \frac{\rho^2(\mu_1^2 -2\mu_1\mu_2+\mu_2^2}{1+\rho^2} \\
    &= \frac{\sigma_1^2}{\sigma_1^2+\sigma_2^2}(\mu_1-\mu_2)^2,\quad\text{and}\\
    (ax-a^{-1}b)^2 &= (1+\rho^2)\left(x - \frac{\sigma_2^2\mu_1 + \sigma_1^2\mu_2}{\sigma_1^2+\sigma_2^2}\right).
    \end{split}
  \end{equation}
  Substituting \eqref{Q(x):2} into \eqref{Q(x):1}, we get
  \begin{align*}
    Q(x) &= \frac{(x-\mu)^2}{2\sigma^2} + \frac{(\mu_1-\mu_2)^2}{2(\sigma_1^2+\sigma_2^2)},\quad\text{where}\\
    \mu :&= \frac{\sigma_2^2\mu_1 + \sigma_1^2\mu_2}{\sigma_1^2+\sigma_2^2},\quad\text{and}\\
