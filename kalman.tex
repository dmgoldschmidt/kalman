...
\begin{Lem}\label{dQdA}
  ...
  \begin{align*}
  Q :&= \mu^TA^TSA\nu,\quad\text{then}\\
  \frac{\partial{Q}}{\partial{A}} &= 2SA\mu\nu^T.
  \end{align*}
\end{Lem}
\begin{proof}
Expanding, we have
...
of $\overline{M}^TS_M\overline{M}$ are all equal to $1$.  To do this, we make an orthonormal change of
...
$$
E_1 + \sum_{j=1}^n\lambda_j\left(1 - \sum_{i=1}^m\bar{m}_{ij}S_{ik}\bar{m}_{kj}\right)
$$
...
2S_M\overline{M}\Lambda,
\end{align*}
Multiplying through by $\frac{1}{2}S_M^{-1}$, we find that in the unconstrained case, putting $U = I$, we get
...
\begin{align*}
  \lambda S_T^{-1}\overline{T}^{-1} &= \sum_{t=1}^N\left(\overline{T}S_{t-1,t}^{-1} - I_{1,t} + \overline{T}I_{2,t}\right)\\
