...
\section{The Alpha Pass}
  ...
  First, we define
\begin{align*}
  P_{a,t} &:= Pd(x_1,x_2,\dots,x_t \mid \theta),\quad\text{for $1\le m \le n\le T$},\quad\text{and}\\
  P_{a,0} &:= 1.
\end{align*}
...
\begin{proof}
  ...
  \begin{align*}
    \hat{\beta}_{T-1}(u) &= \N(u;x_{T-1},\Sigma_{Ob}),\quad\text{and}\\
    \beta_{T-1}(s) &= \int_{\R^n}\N(u;x_{T-1},\Sigma_{Ob})\N(u;s,\Sigma_{Tr})du \\
    &= \N(s;x_{T-1},\Sigma_{Ob}+\Sigma_{Tr})
  \end{align*}

  Thus, our initial values at $t = T-1$ are
  \begin{align*}
  P_{b,T-1} &= 1,\\
  \mu_{b,T-1} &= x_{t-1}, \quad\text{and}\\
