...
of $\overline{M}^T\overline{S}_M\overline{M}$ are all equal to $1$.  To do this, we make an orthonormal change of
state variable $s_t := U_t\sigma_t$, where $U_t$ will be determined below. Thus
$$
\gamma_t(\sigma_t) = \N(\sigma_t;U_t^T\mu_{c,t},U_t^TS_{c,t}U_t).
$$
Then we differentiate
$$
E_1 + \sum_{j=1}^n\lambda_j\left(1 - \sum_{i,k=1}^m\bar{m}_{ij}\overline{S}_{ik}\bar{m}_{kj}\right)
$$
...
\begin{align*}
  0  &= 2\sum_{t=1}^N\int_{\R^n}\gamma_t(\sigma_t)(\overline{S}_Mx_t\sigma_t^TU_t^T - \overline{S}_M\overline{M}U_t\sigma_t\sigma_t^TU_t^T)d\sigma_t +
  2\overline{S}_M\overline{M}\Lambda\\
...
where 
$$
\lambda_j^* = (U^T\Gamma_1^T\overline{S}_M\Gamma_1U)_{jj}^{-\frac{1}{2}}\quad(1\le j\le n).
$$
