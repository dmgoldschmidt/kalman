...
E_1 &= N\log|\overline{S}_M| - \sum_{t=1}^N\int_{\R^n}\gamma_t(s_t)Q(x_t;Ms_t,\overline{S}_M)ds_t.
\end{align*}
...
$$
E_2 = N\log|\overline{S}_T| - \sum_{t=1}^N\int_{\R^{2n}}\hat{\gamma}_t(s_{t-1},s_t)Q(s_t;Ts_{t-1},\overline{S}_T)ds_{t-1}ds_t,
$$
...
\begin{align*}
%  - \sum_{t=1}^N\int_{\R^n}\gamma_t(s')\frac{\partial{Q(x_t;\overline{M}Us',\overline{S}_M)}}{\partial{\overline{M}}}ds
  0  &= 2\sum_{t=1}^N\int_{\R^n}\gamma_t(s_t')(\overline{S}_Mx_ts_t'^TU_t^T - \overline{S}_M\overline{M}U_ts_t's_t'^TU_t^T)ds' +
  2S_M\overline{M}\Lambda\\
&= 2\sum_{t=1}^N(\overline{S}_Mx_t\mu_{c,t}^TU - \overline{S}_M\overline{M}U^T(S_{c,t}^{-1} + \mu_{c,t}\mu_{c,t}^T)U +
2S_M\overline{M}\Lambda,
\end{align*}
...
\begin{equation}\label{Sigma_Tr:0}
  ...
  &= (2-\delta_{ij})\overline{T}T\overline{S}_T^{-1} - (2-\delta{ij}) \sum_{t=1}^N\int_{\R^{2n}}\N(s_{t-1};\mu_{t-1,t},S_{t-1,t})\gamma_t(s_t)(s_{t-1}-s_t)
...
depends on $s_t$, namely  
\begin{equation}\label{s-mu_1}
  \begin{split}
    s_t-\mu_{t-1,t} &= (I - S_{t-1,t}^{-1}S_T)s_t - S_{t-1,t}^{-1}S_{a,t-1}\mu_{a,t-1}, \\
...
\begin{equation}\label{S_Tr:2}
  \begin{split}
  \overline{S}_T^{-1} &=\frac{1}{N}\sum_{t=1}^N\left(S_{t-1,t}^{-1} +S_{t-1,t}^{-1}S_{a,t-1}
  \left[\int_{-\infty}^{\infty}\gamma_t(s_t)(s_t-\mu_{a,t-1})(s_t-\mu_{a,t-1})^Tds_t\right]S_{a,t-1}S_{t-1,t}^{-1}\right)\\
   &=\frac{1}{N}\sum_{t=1}^NS_{t-1,t}^{-1}+S_{t-1,t}^{-1}S_{a,t-1}
...
$$
where $\lambda$ is a Lagrange multiplier, and then imposing the constraint $|\overline{T}| = 1$.

Thus, we have
\begin{align*}
\lambda|\overline{T}|\overline{T}^{-T} &= \sum_{t=1}^N\int_{\R^{2n}}\hat{\gamma}_t(s_{t-1},s_t)
...
&= -2\overline{S}_T\left(\sum_{t=1}^N\int_{\R^{n}}\gamma_t(s_t) (s_t\mu_{t-1,t}^T -
\overline{T}(S_{t-1,t}^{-1} + \mu_{t-1,t}\mu_{t-1,t}^T))ds_t\right) \\
\end{align*}
