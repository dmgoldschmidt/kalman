...
which we denote by $\hat{\gamma}_t(s,u)$. The joint probability density of all the data and the two given states
  ...
  \begin{align}
    ...
    \Sigma^{(2)}_t &:= \widehat{\Sigma}_{a,t}(\widehat{\Sigma}_{a,t}+\widetilde{\Sigma}_{b,t-1})^{-1}\widetilde{\Sigma}_{b,t-1},     \quad\text{and}\notag\\
    \mu^{(2)}_t &:= \widehat{\Sigma}_{a,t}(\widehat{\Sigma}_{a,t}+\widetilde{\Sigma}_{b,t-1})^{-1}\mu_{b,t-1} +
    \widetilde{\Sigma}_{b,t-1}(\widehat{\Sigma}_{a,t} + \widetilde{\Sigma}_{b,t-1})^{-1}\mu_{a,t-1}.\notag
  \end{align}
...
$$
\frac{\partial{\det(A)}}{\partial{A}} = A^* = \det(A)A^{-1},\quad\text{and}\quad\frac{\partial{\log\det(A)}}{\partial{A}} = A^{-1}. 
$$
...
&= -\overline{\Sigma}_0^{-1} + \int_{\R^n}\gamma_0(s)(s-\mu_{c,0})(s-\mu_{c,0})^Tds.
\end{align*}
...
\overline{\Sigma}_0^{-1} = \Sigma_{c,0}.
\end{equation}
...
\begin{align*}
  ...
  &= -T\overline{\Sigma}_{Ob}^{-1} + \sum_{t=1}^T\int_{\R^n}\gamma_t(s)(s-x_t)(s-x_t)^Tds\\
  &= -T\overline{\Sigma}_{Ob}^{-1} + \sum_{t=1}^T\int_{\R^n}\gamma_t(s)(ss^T - sx_t^T - x_ts^T + x_tx_t^T)ds \\
  &= -T\overline{\Sigma}_{Ob}^{-1} + \sum_{t=1}^T(\overline{\Sigma}_{c,t} + \mu_{c,t}\mu_{c,t}^T -\mu_{c,t}x_t^T- x_t\mu_{c,t}^T + x_tx_t^T).
\end{align*}
Thus, we see that
\begin{equation}\label{Sigma_Ob}
  \overline{\Sigma}_{Ob}^{-1} = \frac{1}{T}\sum_{t=1}^T [\Sigma_{c,t} + (\mu_{c,t}-x_t)(\mu_{c,t}-x_t)^T].
\end{equation}

Finally, we minimize $E_2$ by differentiating with respect to $\overline{\Sigma}_{Tr}^{-1}$:
\begin{equation}\label{Sigma_Tr:0}
  \begin{split}
  0 = \frac{\partial{E_2}}{\partial{\overline{\Sigma}_{Tr}^{-1}}} &= -T\frac{\partial{\log\det(\overline{\Sigma}_{Tr}^{-1})}}{\partial\overline{\Sigma}_{Tr}^{-1}}
  + \sum_{t=1}^T\int_{\R^{2n}}\hat{\gamma}_t(s,u)\frac{\partial{Q(s;u,\overline{\Sigma}_{Tr}^{-1})}}{\partial\overline{\Sigma}_{Tr}^{-1}}duds \\
      &= -T\overline{\Sigma}_{Tr}^{-1} + \sum_{t=1}^T\int_{\R^{2n}}\N(u;\mu^{(1)}_t,\Sigma^{(1)}_t)\N(s;\mu^{(2)}_t\Sigma^{(2)}_t)(u-s)(u-s)^Tduds.
  \end{split}
\end{equation}
...
\begin{align*}
  ...
  &= \Sigma^{(1)}_t + (s-\mu^{(1)}_t)(s-\mu^{(1)}_t)^T.
\end{align*}
Substituting this result into \eqref{Sigma_Tr:0}, we get
\begin{equation}\label{Sigma_Tr:1}
  \overline{\Sigma}_{Tr}^{-1} = \frac{1}{T}\sum_{t=1}^T\int_{-\infty}^{\infty}N(s;\mu^{(2)}_t,\Sigma^{(2)}_t)(\Sigma^{(1)}_t+
...
Fortunately, neither $\mu^{(2)}_t$ nor $\Sigma^{(2)}_t$ depends on $s$, so after the substitution $s_1 = s - \mu_{a,t-1}$,
\eqref{Sigma_Tr:1} becomes
\begin{equation}\label{Sigma_Tr:2}
  \begin{split}
    \overline{\Sigma}_{Tr}^{-1} &= \frac{1}{T}\sum_{t=1}^T\Sigma^{(1)}_t + \frac{1}{T}\sum_{t=1}^T
    \int_{-\infty}^{\infty}\N(s_1;\mu^{(2)}_t-\mu_{a,t-1},\Sigma^{(2)}_t)\Sigma_{Tr}\widehat{\Sigma}_{a,t}^{-1}s_1
    s_1^T\widehat{\Sigma}_{a,t}^{-1}\Sigma_{Tr}ds_1\\
    &=\frac{1}{T}\sum_{t=1}^T\Sigma^{(1)}_t +\frac{1}{T}\sum_{t=1}^T\Sigma_{Tr}\widehat{\Sigma}_{a,t}^{-1}
    \left[\int_{-\infty}^{\infty}\N(s_1;\mu^{(2)}_t-\mu_{a,t-1},\Sigma^{(2)}_t)s_1s_1^Tds_1\right]\widehat{\Sigma}_{a,t}^{-1}\Sigma_{Tr}\\
    &=\frac{1}{T}\sum_{t=1}^T\Sigma^{(1)}_t+\frac{1}{T}\sum_{t=1}^T\Sigma_{Tr}\widehat{\Sigma}_{a,t}^{-1}
    [\Sigma^{(2)}_t+(\mu^{(2)}_t-\mu_{a,t-1})(\mu^{(2)}-\mu_{a,t-1})^T]\widehat{\Sigma}_{a,t}^{-1}\Sigma_{Tr}.
  \end{split}
\end{equation}
