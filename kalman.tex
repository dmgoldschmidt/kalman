...
$$
Q_0 := \mu^TA^TSA\mu = \sum_{i,j,k,l}\mu_iA_{ji}S_{jk}A_{kl}\mu_l.
$$
Collecting terms which involve $A_{qr}$, if $j=q$ and $i=r$, we get
\begin{equation}\label{SA}
A_{qr}\mu_r\sum_{k,l}S_{qk}A_{kl}\mu_l,
\end{equation}
while if $k=q$ and $l=r$, we get
\begin{equation}\label{A^TS}
A_{qr}\mu_r\sum_{i,j}\mu_iA_{ji}S_{jq}.
\end{equation}
However after replacing $i$ by $l$, $j$ by $k$ and using the fact that $S$ is symmetric,
\eqref{SA} and \eqref{A^TS} are identical. But they both have the common term
$\mu_r^2A_{qr}^2S_{qq}$ when $j = k = q$ and $i = l = r$.
We'll call this term the exceptional term. Then
\begin{align}
  \notag\frac{\partial{Q_0}}{\partial{A_{qr}}} &= 2\mu_r\sum_{(k,l)\neq(q,r)}S_{qk}A_{kl}\mu_l + 2\mu_r^2A_{qr}S_{qq}\\
  &= 2\mu_r\sum_{k,l}S_{qk}A_{kl}\mu_l. \label{dQ_0}
\end{align}
So the exceptional term isn't really exceptional after all.

We conclude that the matrix whose $(q,r)$ term is given by \eqref{dQ_0} is
$$
\frac{\partial{Q_0}}{\partial{A}} = 2SA\mu\mu^T
$$
as asserted.
\end{proof}
...
\begin{align*}
  I_1 :&= \int_{\R^n}\gamma_t(s_t)s_t\mu_{t-1,t}^Tds_t \\
...
and
\begin{align*}
  I_2 :&= \int_{\R^n}\gamma_t(s_t)\mu_{t-1,t}\mu_{t-1,t}^Tds_t \\
