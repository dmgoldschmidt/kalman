...
%    &= P_{b,t+1}(2\pi)^{-n}(|S_x||S_{b,t+1}|)^{\frac{1}{2}}\exp\{-\frac{1}{2}(Q(u;\hat{\mu}_{b,t+1},\hat{S}_{b,t+1}) + R_{b,t})\}\\
  ...
  \end{align*}
  $\hat{\beta}_{t+1}(u)$ is the conditional likelihood of state $u$ at time $t+1$ given all observations $x_\tau~
  (\tau \ge t+1)$.  Then an application of \eqref{exact_prod} and \eqref{comp_sq:4} yields the time update
  \begin{align*}
...
$$
Substituting this into \eqref{E2:1}and using \eqref{E_x^tSx} with $\widehat{S}_T := (T-\overline{T})^tS_T(T-\overline{T})$, we get
\begin{equation}\label{E2:2}
  \begin{split}
    E_2 &= N\log|S_T| - \sum_{t=1}^N\int_{\R^n} \tr(\overline{S}_T^{-1}S_T) + s_{t-1}^t\widehat{S}_Ts_{t-1}\gamma_{t-1}(s_{t-1})ds_{t-1} \\
       &= N\log|S_T| - N\tr(\overline{S}_T^{-1}S_T) - \sum_{t=1}^NE_{\gamma_{t-1}(s_{t-1})}  s_{t-1}^t\widehat{S}_Ts_{t-1}  \\
       &= N\log|S_T| - N\tr(\overline{S}_T^{-1}S_T)  -\sum_{t=1}^N\tr(S_{c,t-1}^{-1}\widehat{S}_T) + \mu_{c,t-1}^t\widehat{S}_T\mu_{c,t-1}.
  \end{split}
\end{equation}
...
%N\log|S_T| - N\tr(\overline{S}_T^{-1}S_T) -\sum_{t=1}^N \tr(S_{c,t-1}^{-1}S_T) + \mu_{c,t-1}^t(\Delta{T})^tS_T(\Delta{t})\mu_{c,t-1}
We first expand $\mu_{c,t-1}^t\widehat{S}_T\mu_{c,t-1}$:
\begin{align*}
  \mu_{c,t-1}^t\widehat{S}_T\mu_{c,t-1} &= \mu_{c,t-1}^t(T-\overline{T})^tS_T(T-\overline{T})\mu_{c,t-1}\\
                                         &= \mu_{c,t-1}^tT^tS_TT\mu_{c,t-1} - \mu_{c,t-1}^tT^tS_T\overline{T}\mu_{c,t-1}\\ 
                                          &-  \mu_{c,t-1}^t\overline{T}^tS_TT\mu_{c,t-1}
                                           + \mu_{c,t-1}^t\overline{T}^tS_T\overline{T}\mu_{c,t-1}.
\end{align*}

Then using \eqref{dQdA}:
\begin{equation}\label{dQdA:2}
  \begin{split}
  \frac{\partial}{\partial{T}} [\mu_{c,t-1}^t\widehat{S}_T\mu_{c,t-1}] &= \frac{\partial}{\partial{T}}
                                                                        [\mu_{c,t-1}^tT^tS_TT\mu_{c,t-1} 
                                                                     - 2 \mu_{c,t-1}^tT^tS_T\overline{T}\mu_{c,t-1}]\\
                                                                 &= 2S_TT\mu_{c,t-1}\mu_{c,t-1}^t - 2S_T\overline{T}\mu_{c,t-1}\mu_{c,t-1}^t\\
                                                                       &= 2S_T(T-\overline{T})\mu_{c,t-1}\mu_{c,t-1}^t. 
  \end{split}
\end{equation}

Next, using \eqref{trA^tSB} and \eqref{trA^tSAB}:
\begin{equation}\label{d_tr_prod:2}
  \begin{split}
\frac{\partial}{\partial{T}}\tr(S_{c,t-1}^{-1}\widehat{S}_T) &= \frac{\partial}{\partial{T}}\tr([T^tS_TT 
                                                               - 2T^tS_T\overline{T}]S_{c,t-1}^{-1})\\
    &= 2S_T(T - \overline{T})S_{c,t-1}^{-1}
  \end{split}
\end{equation}

Finally, we optimize \eqref{E2:2}, dropping terms that do not involve $T$ and using \eqref{x^tAy}, \eqref{dQdA:2}, 
and \eqref{d_tr_prod:2}: 
\begin{equation}\label{E2:3} 
  \begin{split}
    0 &= \frac{\partial}{\partial{T}}\left(N\log|S_T| - N\tr(\overline{S}_T^{-1}S_T)  -\sum_{t=1}^N\tr(S_{c,t-1}^{-1}\widehat{S}_T) + \mu_{c,t-1}^t\widehat{S}_T\mu_{c,t-1}\right)\\
      &= -\frac{\partial}{\partial{T}}\left(\sum_{t=1}^N\tr(\widehat{S}_T(S_{c,t-1}^{-1} + \mu_{c,t-1}\mu^t_{c,t-1}))
        \right)\\
      &=  -\frac{\partial}{\partial{T}}\left(\sum_{t=1}^N\tr(T^tS_TT - 2T^tS_T\overline{T})(S_{c,t-1}^{-1} + \mu_{c,t-1}\mu^t_{c,t-1}))
        \right)\\
    &= -2S_T(T-\overline{T})\sum_{t=1}^N(S_{c,t-1}^{-1} + \mu_{c,t-1}\mu_{c,t-1}^t)
  \end{split}
\end{equation}
Solving for $T$, we define
$$
X_c := \sum_{t=1}^N(S_{c,t-1}^{-1} + \mu_{c,t-1}\mu_{c,t-1}^t)
$$
and we get
\begin{equation}\label{T}
  S_TTX_c = S_T\overline{T}X_c.
\end{equation}
