...
\end{equation}
\begin{proof}
  The exponential terms on both sides of the equation are equal by \eqref{comp_sq:1} and the scale factors
...
\begin{align*}
  ...
  P_{b,N} :&= 1;
\end{align*}
  For $t <  N$, we have the measurement update 
  \begin{align*}
  ...
  \begin{align*}
    ...
    &= P_{b,t+1}N_{b,t}\int_{\R^n}\N(u;\hat{\mu}_{b,t},\hat{S}_{b,t+1})\N(Tu;s,S_T)du\\
    &=P_{b,t+1}N_{b,t}\N(T^{-1}s;\hat{\mu}_{b,t},\hat{S}_{b,t+1}*S_T')\\
    &=P_{b,t+1}N_{b,t}\N(s;T\hat{\mu}_{b,t},T^{-T}(\hat{S}_{b,t+1}*S_T')T)\\
...
Note that the constraint equation does not depend on $t$. Setting the derivative to zero we must solve for $\overline{M}$:
\begin{align*}
  0 &= \frac{\partial{E_1}}{\partial\overline{M}} + 2\overline{S}_M\overline{M}\Lambda\\
  &= -\sum_{t=1}^N\int_{\R^n}\gamma_t(\sigma_t)\frac{\partial}{\partial\overline{M}}Q(x_t,\overline{M}U_t\sigma_t,\overline{S}_M)d\sigma_t +   2\lambda\overline{S}_M\overline{M}\\
  &= 2\sum_{t=1}^N\int_{\R^n}\gamma_t(\sigma_t)(\overline{S}_Mx_t\sigma_t^TU_t^T - \overline{S}_M\overline{M}U_t\sigma_t\sigma_t^TU_t^T)d\sigma_t +
  2\lambda\overline{S}_M\overline{M}\\
&= 2\sum_{t=1}^N(\overline{S}_Mx_t\mu_{c,t}^TU_t - \overline{S}_M\overline{M}U_t^T(S_{c,t}^{-1} + \mu_{c,t}\mu_{c,t}^T)U_t +
2\lambda\overline{S}_M\overline{M}.
\end{align*}
Multiplying through by $\frac{1}{2}\overline{S}_M^{-1}$, we find that in the unconstrained case, with $U = I$, we get
...
Since $\Gamma_2$ is symmetric positive semi-definite, there is an orthonormal matrix $U$
such that
\begin{align*}
  D :&= U^T\Gamma_2U = \diag\{d_1,\dots,d_n\}.\quad\text{Define} \\
  \widehat{M} :&= U^T\overline{M}U,\\
  \widehat{\Gamma}_1 :&= U^T\Gamma_1U,\quad\text{and}\\
  \widehat{S}_M :&= U^T\overline{S}_MU.
\end{align*}
...
\begin{align*}
1 &= \tr(\overline{M}^T\overline{S}_M\overline{M}) = \tr(U^T\overline{M}^T\overline{S}_M\overline{M}U)\\
&= \tr(\widehat{M}^T\widehat{S}_M\widehat{M})\\
&= \tr((D+\lambda{I})^{-1}\widehat {\Gamma}_1^T\widehat{S}_M\widehat{\Gamma}_1(D+\lambda{I})^{-1})\\
&= \tr((D+\lambda{I})^{-2}\widehat{\Gamma}_1^T\widehat{S}_M\widehat{\Gamma}_1).
\end{align*}
Define
$$
f(\lambda) = 1 - \sum_{i=1}^n\frac{\sigma_{ii}}{(d_i+\lambda)^2},
$$
where $\sigma_{ii}$ is the $i^{th}$ diagonal element of $\widehat{\Gamma}_1^T\widehat{S}_M\widehat{\Gamma}_1$.
Then we solve $f(\lambda) = 0$ for $\lambda$ by e.g. Newton's method and plug the result into \eqref{M_bar:2}. 
