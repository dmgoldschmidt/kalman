...
which we denote by $\hat{\gamma}_t(s,u)$. The joint probability density of all the data and the two given states
  ...
  \begin{align}
   ...
   \mu^{(1)}_t &:= (S_t^{(1)})^{-1}(S_{Tr}s + S_{a,t-1}\mu_{a,t-1}),\label{mu(1)}\\
    ...
    \mu^{(2)}_t &:= (S_t^{(2)})^{-1}(S_{Tr}*S_{a,t-1}\mu_{a,t-1} + (M^TS_{Ob}M+S_{b,t})\mu_{b,t-1})\label{mu(2)}.
  \end{align}
...
$$
\frac{\partial{Q(x;M\mu,S)}}{\partial{M}}.
$$

\begin{Lem}\label{dM}
  $$
  \frac{\partial{Q(x;M\mu,S)}}{\partial{M}} = -2SMx\mu^T + 2SM\mu\mu^T.
  $$
\end{Lem}
\begin{proof}
Expanding, we have
