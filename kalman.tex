...
which we denote by $\hat{\gamma}_t(s,u)$. The joint probability density of all the data and the two given states
  ...
  \begin{align}
    ...
    &\propto \N(u;\mu_{u,t},S_{u,t})\N(s;\mu_{a,t-1},\hat{S}_{a,t-1})\N(s;\mu_{b,t-1},\hat{S}_{b,t-1})\notag\\
  ...
  \begin{equation}\label{gamma_su}
    \begin{split}
      ...
      S_{s,t} &:= \hat{S}_{a,t-1} + \hat{S}_{b,t-1},\quad\text{and}\\
      \mu_{s,t} &:= S_{s,t}^{-1}(\hat{S}_{a,t-1}\mu_{a,t-1} + \hat{S}_{b,t-1}\mu_{b,t-1}). 
    \end{split}
  \end{equation}
...
\begin{align}
  ...
  \left[\int_{-\infty}^{\infty}\N(s;\mu_{s,t},S_{s,t})(s-\mu_{a,t-1})(s-\mu_{a,t-1})^Tds\right]\hat{S}_{a,t-1}\right)S_{Tr}^{-1}
  \notag\\
   &=\frac{1}{T}\sum_{t=1}^TS_{u,t}^{-1}+\frac{1}{T}S_{Tr}^{-1}\left(\sum_{t=1}^T\hat{S}_{a,t-1}
   [S_{s,t}^{-1}+(\mu_{s,t}-\mu_{a,t-1})(\mu_{s,t}-\mu_{a,t-1})^T]\hat{S}_{a,t-1}\right)S_{Tr}^{-1}.\label{S_Tr:2}
  \end{align}

Finally, we can make the following simplification by using \eqref{gamma_su}.
\begin{align*}
  \mu_{s,t} - \mu_{a,t-1} &= (S_{s,t}^{-1}\hat{S}_{a,t-1} - I)\mu_{a,t-1} + S_{s,t}^{-1}\hat{S}_{b,t-1}\mu_{b,t-1}\\
  &= (S_{s,t}^{-1}\hat{S}_{a,t-1} - S_{s,t}^{-1}S_{s,t})\mu_{a,t-1} + S_{s,t}^{-1}\hat{S}_{b,t-1}\mu_{b,t-1}\\
...
Define
$$
v_{a,b,t} := S_{s,t}^{-1}\hat{S}_{b,t-1}(\mu_{b,t-1}-\mu_{a,t-1}).
$$
Then we have 
\begin{equation}\label{S_Tr:3}
  \begin{split}
\overline{S}_{Tr}^{-1} = \frac{1}{T}\sum_{t=1}^TS_{u,t}^{-1} + S_{Tr}^{-1}\left(\frac{1}{T}\sum_{t=1}^T\hat{S}_{a,t-1}[
S_{s,t}^{-1} + v_{a,b,t}v_{a,b,t}^T]\hat{S}_{a,t-1}\right)S_{Tr}^{-1}.
\end{split}
\end{equation}  
\end{document}
