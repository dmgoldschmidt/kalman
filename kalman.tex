...
$$
  P(s_{t-1},s_t) = \alpha_{t-1}(s_{t-1})\hat{\beta}_t(s_t) = \N(s_{t-1}; \mu_{a,t-1},S_{a,t-1})\N(s_t;\hat{\mu}_{b,t},\hat{S}_{b,t}),
  $$
...
Since the observations are assumed independent, and since we have omitted the transition probability from $s_{t-1}$ to $s_t$ (which is why we have used $\hat{\beta}_t$ instead of $\beta_t$), we can multiply the two densities.  
\end{proof}
...
\begin{equation}\label{E2:1}
  \begin{split}
    E_2 &= N\log|S_T| - \sum_{t=1}^N\int_{R^{2n}}Q(s_t;Ts_{t-1},S_t)P(s_{t-1},s_t)ds_{t-1}ds_t\\
        &= N\log|S_T| - \sum_{t=1}^N\int_{R^n}\left(\int_{\R^{n}}Q(s_t;Ts_{t-1},S_T)\hat{\beta}_t(s_t)ds_t\right)
          \alpha_{t-1}(s_{t-1})ds_{t-1}\\
  \end{split}
  \end{equation}

In order to evaluate the inner integral in \eqref{E2:1}, we use \eqref{E_Q} 
with $\gamma := \hat{\beta}_t(s_t) = \N(s_t;\hat{\mu}_{b,t},\hat{S}_{b,t})$,  $A = I$,   $ \mu = Ts_{t-1}$, and
...
\begin{equation}\label{E2:2}
  \begin{split}
    E_2 &= N\log|S_T| - \sum_{t=1}^N\int_{\R^n} \tr(S_{b,t}^{-1}S_T) + Q(Ts_{t-1};\hat{\mu}_{b,t},S_T)\alpha_{t-1}(s_{t-1})ds_{t-1}\\
      &= N\log|S_T| - N - \sum_{t=1}^N\tr(S_{b,t}^{-1}S_T)E_{\alpha_{t-1}(s_{t-1})}Q(Ts_{t-1};\hat{\mu}_{b,t},S_T) \\
      &= N\log|S_T|  - \sum_{t=1}^N\tr(S_{b,t}^{-1}S_T)\left(\tr(S_{a,t-1}^{-1}T^tS_TT) + Q(\hat{\mu}_{b,t};T\mu_{a,t-1},S_T)\right).
  \end{split}
\end{equation}
...
\eqref{dM}.  Define $\tau_t := \tr(S_{b,t}^{-1}S_T)$, then we have 
  \begin{align*}
    0 &= \frac{\partial}{\partial{T}} \left(N\log|S_T|  - \sum_{t=1}^N\tr(S_{b,t}^{-1}S_T)[\tr(S_{a,t-1}^{-1}T^tS_TT) + Q(\hat{\mu}_{b,t};T\mu_{a,t-1},S_T)]\right)\\
    &= - \sum_{t=1}^N\tr(S_{b,t}^{-1}S_T)\frac{\partial}{\partial{T}}[\tr(S_{a,t-1}^{-1}T^tS_TT) + Q(\hat{\mu}_{b,t};T\mu_{a,t-1},S_T)]\\
      &= - \sum_{t=1}^N\tau_t[2S_TTS_{a,t-1}^{-1} + 2S_T(T\mu_{a,t-1} - \mu_{b,t})\mu_{a,t-1}^t]\\
       &= -2S_T\sum_{t=1}^NT\tau_t[S_{a,t-1}^{-1} + \mu_{a,t-1}\mu_{a,t-1}^t] - \tau_t\mu_{b,t}\mu_{a,t-1}^t
  \end{align*}

Solving for $T$, we can multiply through by $(-2S_T)^{-1}$ since $S_T$ is positive definite, and we get
\begin{equation}\label{E2:3}
  T = \Gamma_1\Gamma_2^{-1},
\end{equation}
where
\begin{align*}
 \Gamma_1 :&= \sum_{t=1}^N\tau_t\mu_{b,t}\mu_{b,t}^t, \quad\text{and}\\
