...
To compact our notation a little,  we define the quadratic form
\begin{align*}
  Q(x;\mu,\Sigma) :&= (x-\mu)^T\Sigma(x-\mu), \quad\text{then}\\
  \log{\N(x;\mu,\Sigma)} &= -\frac{1}{2}\{\log{\det(2\pi\Sigma)} + Q(x;\mu,\Sigma^{-1})\}.
\end{align*}
  
Thus, we have
\begin{align}
  E(\theta',\theta) &= -\frac{1}{2}(E_0 + E_1 + E_2),\quad\text{where}\notag\\
  E_0 :&= \int_{\R^{Tn}}P(X,S\mid\theta')\{\log{\det(2\pi\Sigma_0)} + Q(s_0;\mu_0,\Sigma_0^{-1})\}dS,\label{E0}\\
  E_1 :&= \int_{\R^{Tn}}P(X,S\mid\theta')\sum_{t=1}^T\log{\det(2\pi\Sigma_{Ob})} + Q(x_t;s_t,\Sigma_{Ob}^{-1})dS,
\label{E1}\\
  E_2 :&= \int_{\R^{Tn}}P(X,S\mid\theta')\sum_{t=1}^T\log{\det(2\pi\Sigma_{Tr})} + Q(s_t;s_{t-1},\Sigma_{Tr}^{-1})dS,
\label{E2}
\end{align}
Since none of the parameters of $\theta$ appear in more than one of these three expressions, we can maximize $E$
by minimizing each of $E_1,E_2,E_3$ separately. Moreover, since the expected value of a constant is just that constant, 
and since the only component of $S$ in the integrand of \eqref{E0} is $s_0$, the expected value collapses to the marginal 
...
\begin{align*}
E_0 &= \log\det(2\pi\Sigma_0) + \int_{\R^n}\gamma_0(s)Q(s;\mu_0,\Sigma_0)ds,\quad\text{and}\\
E_1 &= \log\det(2\pi\Sigma_{Ob}) + \sum_{t=1}^T\int_{\R^n}\gamma_t(s)Q(x_t;s,\Sigma_{Ob})ds.
\end{align*}
...
$$
E_2 = \log\det(2\pi\Sigma_{Ob}) + \sum_{t=1}^T\int_{\R^n}\hat{\gamma}_t(s,u)Qs;u,\Sigma_{Tr})ds,
$$
where $\hat{\sigma}_t(s,u)$ is the joint posterior probability density of state $s=u$ at time $t-1$ and state $s$ 
at time $t$.
...
o reestimate $M$ and $b$? Use least-squares!
\|
...
\begin{align*}
\log{P_{t,1}} &= \log{P_{1,t-1}}+\frac{n}{2}\log(2\pi)+
\frac{1}{2}\log\det(\Sigma_{Tr}\widehat{\Sigma}_t^{-1}\Sigma_{a,t-1})\\
&-\frac{n}{2}\log(2\pi) - \frac{1}{2}\log\det(\Sigma_{Ob}+\widehat{\Sigma}_t)
-\frac{1}{2}Q(x_t;\mu_{a,t-1},\Sigma_{Ob}+\widehat{\Sigma}_t)\\
&= \log{P_{t-1}} + \frac{1}{2}\log\det(\Sigma_{Tr}\widehat{\Sigma}_t^{-1}\Sigma_{a,t-1}) - \frac{1}{2}\log\det(\Sigma_{Ob}+\widehat{\Sigma}_t)\\
&-\frac{1}{2}Q(x_t;\mu_{a,t-1},\Sigma_{Ob}+\widehat{\Sigma}_t)\\
\end{align*}
