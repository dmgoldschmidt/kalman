...
\begin{align*}
  ...
  \mu_{u,t} &:= S_{u,t}^{-1}(T^TS_Ts + S_{a,t-1}\mu_{a,t-1}).
\end{align*}
\end{Lem}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\propto \N(u;\mu_{a,t-1},S_{a,t-1})\N(Tu;s,S_T)\N(x_t;Ms,S_M)\N(s;\mu_{b,t},S_{b,t}).
  \end{align*}
  Combining the first two factors using \eqref{comp_sq:2} yields
  \begin{equation}\label{gamma_hat:1}
    \hat{\gamma}_t(u,s) = \N(u;\mu_{u,t},S_{u,t})\N(T^{-1}s;\mu_{a,t-1},\hat{S}_{a,t-1})\N(Ms,x_t,S_M)\N(s;\mu_{b,t},S_{b,t}).
  \end{equation}
...
\end{proof}
  ...
  $$
  because the second factor does not depend on $u$, so \eqref{gamma_hat} is a bona-fide probability density and no
...
state variable $s := Us'$, where $U$ will be determined below. Thus
$$
\gamma_t(s') = \N(s';U^T\mu_{c,t},U^TS_{c,t}U).
$$
Then we differentiate
$$
E_1 + \sum_{j=1}^n\lambda_j\left(1 - \sum_{i=1}^m\bar{m}_{ij}S'_{ik}\bar{m}_{kj}\right)
$$
with respect to $\overline{M}$, where the $\lambda_j$ are Lagrange multipliers.
Put $\Lambda = diag\{\lambda_1,\dots,\lambda_n\}$.  Then setting the derivative to zero, we must solve
\begin{align*}
%  - \sum_{t=1}^T\int_{\R^n}\gamma_t(s')\frac{\partial{Q(x_t;\overline{M}Us',\overline{S}_M)}}{\partial{\overline{M}}}ds
  0  &= 2\sum_{t=1}^T\int_{\R^n}\gamma_t(s')(\overline{S}_Mx_ts'^TU^T - \overline{S}_M\overline{M}Us's'^TU^T)ds' +
...
$$
and then set the $\lambda_i$ appropriately to satisfy the constraints. We see that if we choose $U$ to diagonalize
...
for $\overline{M}$ and $\overline{S}_M$ on successive Baum-Welch iterations.
