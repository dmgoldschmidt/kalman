...
\begin{align*}
 ...
 \quad\text{and}\\
  P_{1,t} &=P_{1,t-1}N(x_t;\mu_{a,t-1},\Sigma_{Ob} + \widehat{\Sigma}_t).
\end{align*}
\end{Thm}
...
\begin{align}
 ...
 &= P_{1,t-1}\int_{\R^n}\N(u;s,\Sigma_{Tr})\N(u;\mu_{a,t-1},\Sigma_{a,t-1})du \notag\\
 &= P_{1,t-1}\N(s;\mu_{a,t-1},\widehat{\Sigma}_t).
\end{align}

Then Multiplying by $X(x_t\mid s)$ yields
\begin{align*}
  \alpha_t(s) &= X(x_t\mid s)\hat{\alpha}_t(s) \\
  &= P_{1,t-1}\N(s;x_t,\Sigma_{Ob})\N(s;\mu_{a,t-1},\widehat{\Sigma}_t) \\
  &= P_{1,t-1}\N(x_t;\mu_{a,t-1},\Sigma_{Ob} + \widehat{\Sigma}_t)\N(s;\mu_{a,t},\Sigma_{a,t}), \\
...
\begin{align*}
  ...
  P_{b,t}:&=P_{b,t+1}\det(2\pi\widetilde{\Sigma}_{b,t})^{\frac{1}{2}}\N(x_t;\mu_{b,t+1},\widehat{\Sigma}_{b,t})
\quad(t< T),\\
  P_{b,T} :&= 1.
\end{align*}
\end{Thm}
...
\begin{proof}
  ...
  the special case $\beta_T(u) = 1$.  Thus,
  \begin{align*}
    \hat{\beta}_{T-1}(u) &= \N(u;x_t,\Sigma_{Ob}),\quad\text{and}\\
    \beta_{T-1}(s) &= \int_{\R^n}\N(u;x_t,\Sigma_{Ob})\N(u;s,\Sigma_{Tr})du \\
    &= \N(s;x_t,\Sigma_{Ob}+\Sigma_{Tr})\int_{\R^n}\N(u;\mu,\Sigma)du \\
  \end{align*}
  where $\mu$ is irrelevant, and 
$$
  \Sigma = \Sigma_{Ob}(\Sigma_{Ob}+\Sigma_{Tr})^{-1})\Sigma_{Tr}.
  $$
  Thus, our initial values at $t = T-1$ are
  \begin{align*}
  P_{b,T-1} &= \det(2\pi\Sigma)^{-\frac{1}{2}}, \\
  \mu_{b,T-1} &= x_t, \quad\text{and}\\
  \Sigma_{b,T-1} &= \Sigma_{Ob}+\Sigma_{Tr}.
  \end{align*}
  ...
  \begin{align*}
    ...
    &= P_{b,t+1}\N(x_t;\mu_{b,t+1},\widehat{\Sigma}_{b,t})\N(u;\mu_{b,t},\widetilde{\Sigma}_{b,t}).
  \end{align*}

  Then a second application of \eqref{comp_sq:2} yields
  \begin{align*}
    \beta_t(s) &= \int_{\R^n}\hat{\beta}_t(u)Pd(u\mid s)du \\
    &= P_{b,t+1}\N(x_t;\mu_{b,t+1},\widehat{\Sigma}_{b,t})\int_{\R^n}\N(u;\mu_{b,t},\widetilde{\Sigma}_{b,t})\N(u;s,\Sigma_{Tr})du\\
    &=P_{b,t+1}\N(x_t;\mu_{b,t+1},\widehat{\Sigma}_{b,t})\N(s;\mu_{b,t},\Sigma_{b,t})\int_{\R^n}\N(u;\mu_*,\Sigma_*)du \\
    &= P_{b,t+1}\N(x_t;\mu_{b,t+1},\widehat{\Sigma}_{b,t})\det(2\pi\Sigma_*)^{\frac{1}{2}}
