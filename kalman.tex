...
\end{proof}
  ...
  \section{The Expected Log-Likelihood}
    ...
    As opposed to the discrete HMM case, it turns out that in our case the expected log-likelihood is computable in closed form.
    Although this isn't strictly necessary in order to derive the re-estimation formulas, it serves as a useful check on the
    derivation and on the numerical computation.
    
  \begin{Lem}\label{exp_log}
    Recall the notation of \eqref{comp_sq:1}. Let $N := \N(x;\mu_0,S_0)$ be an $n$-dimensional gaussian, and let $A$ be any
    full rank $n\times{m}$ matrix. Then for any vector $\mu\in\R^n$, 
    \begin{align}
      E_N((x-\mu)(x-\mu)^T) &= S_0^{-1} + (\mu-\mu_0)(\mu-\mu_0)^T, \label{E_xx^T}\\
      E_N((x-\mu)^T(x - \mu)) &=  tr(S_0^{-1}) + (\mu-\mu_0)^T(\mu-\mu_0),\quad\text{and}\label{E_x^Tx}\\
        E_N(Q(Ax;\mu,S)) &=  tr(S_0^{-1}A^TSA) + (\mu-A\mu_0)^TS(\mu-A\mu_0), \label{E_Q}
    \end{align}
  \end{Lem}
  \begin{proof}
    By definition, the covariance matrix for $N$ is
    \begin{align*}
    S_0^{-1} &= E_N(x-\mu_0)(x-\mu_0)^T = E_N(xx^T) - E_N(x\mu_0^T) - E_N(\mu_0{x}^T) + E_N(\mu_0\mu_0^T)\\
    &= E_N(xx^T) - \mu_0\mu_0^T, \quad{whence}\\
    E_N(x-\mu)(x-\mu)^T &= E_N(xx^T) - E_N(x\mu^T) - E_N(\mu{x}^T) + E_N(\mu\mu^T) \\
    ...
    \begin{equation}\label{E_Q:1}
      \begin{split}
      E_N(Q(Ax;\mu,S) &= E_N(x^TA^TSAx) - 2E_N(x^TA^TS\mu) + E_N(\mu^TS\mu) \\
      &= E_N(x^TA^TSAx) - 2\mu_0^TA^TS\mu + \mu^TS\mu.
      \end{split}
    \end{equation}
    
    To evaluate $E_N(x^TA^TSAx)$, we let $C$ be the Cholesky matrix of $A^TSA$, so that
    ...
    of variable $y = Cx,~ dy = |C|dx$ and using \eqref{E_x^Tx}, we then have
      \begin{align*}
        E_N(x^TA^TSAx) &= \int_{R^n}x^TA^TSAx~\N(x;\mu_0,S_0)dx\\
