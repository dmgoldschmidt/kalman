...
which we denote by $\hat{\gamma}_t(s,u)$. The joint probability density of all the data and the two given states
  ...
  \begin{align}
    ...
    \widetilde{\Sigma}_{b,t}(\widehat{\Sigma}_{a,t}+\widetilde{\Sigma}_{b,t})^{-1}\mu_{a,t-1}.\label{mu(2)}
...
\begin{equation} \label{Sigma_tr:2}
  \begin{split}
  \overline{\Sigma}_{Tr} &= \frac{1}{T\det\Sigma_{Tr}}\sum_{t=1}^T\det\widehat{\Sigma}_{a,t}
  \int_{-\infty}^{\infty}N(v;\mu^{(2)}_t-\mu_{a,t-1},\Sigma_t^{(3)})(\Sigma^{(1)}_t + vv^T) dv\\
  &= \frac{1}{T\det\Sigma_{Tr}}\sum_{t=1}^T\det\widehat{\Sigma}_{a,t}\left(\Sigma^{(1)}_t + \Sigma^{(3)}_t
  + (\mu^{(2)}_t-\mu_{a,t-1})(\mu^{(2)}_t-\mu_{a,t-1})^T\right).
  \end{split}
\end{equation}
where
$$
\Sigma_t^{(3)} := \widehat{\Sigma}_{a,t}\Sigma_{Tr}^{-1}\Sigma^{(2)}_t\Sigma_{Tr}^{-1}\widehat{\Sigma}_{a,t}.
$$
...
$$
\Sigma_{ab} = \widehat{\Sigma}_{a,t} + \widetilde{\Sigma}_{b,t}.
$$
Then
\begin{align*}
  \mu^{(2)} - \mu_{a,t-1} &= \widehat{\Sigma}_{a,t}\widehat{\Sigma}_{ab}^{-1}\mu_{b,t-1} +
  (\widetilde{\Sigma}_{b,t-1}\widehat{\Sigma}_{ab}^{-1}-I)\mu_{a,t-1}\\
  &= \widehat{\Sigma}_{a,t}\widehat{\Sigma}_{ab}^{-1}\mu_{b,t-1} +
  (\widetilde{\Sigma}_{b,t-1}\widehat{\Sigma}_{ab}^{-1}-\widehat{\Sigma}_{ab}\widehat{\Sigma}_{ab}^{-1})\mu_{a,t-1}\\
  &=  \widehat{\Sigma}_{a,t}\widehat{\Sigma}_{ab}^{-1}\mu_{b,t-1} +
  (\widetilde{\Sigma}_{b,t-1}-\widehat{\Sigma}_{ab})\widehat{\Sigma}_{ab}^{-1}\mu_{a,t-1}\\
  &= \widehat{\Sigma}_{a,t}\widehat{\Sigma}_{ab}^{-1}(\mu_{b,t-1} -\mu_{a,t-1}),
\end{align*}
and substituting this into \eqref{Sigma_Tr:2} we have
\begin{equation}\label{Sigma_Tr:3}
\overline{\Sigma}_{Tr}   =\frac{1}{T}\sum_{t=1}^T\Sigma^{(1)}_t+\Sigma_{Tr}\left(\frac{1}{T}\sum_{t=1}^T\widehat{\Sigma}_{ab}^{-1}
   [\Sigma^{(2)}_t+(\mu_{b,t-1}-\mu_{a,t-1})(\mu_{b,t-1}-\mu_{a,t-1})^T]\widehat{\Sigma}_{ab}^{-1}\right)\Sigma_{Tr}.
\end{equation}  
