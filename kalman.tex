...
which we denote by $\hat{\gamma}_t(s,u)$. The joint probability density of all the data and the two given states
  ...
  \begin{align}
    ...
    &\propto \N(u;\mu_{u,t},S_{u,t})\N(s;\mu_{a,t-1},\hat{S}_{a,t})\N(s;\mu_{b,t-1},\hat{S}_{b,t-1})\notag\\
  ...
  \begin{equation}\label{gamma_su}
    \begin{split}
      ...
      S_{s,t} &:= \hat{S}_{a,t} + \hat{S}_{b,t-1},\quad\text{and}\\
      \mu_{s,t} &:= S_{s,t}^{-1}(\hat{S}_{a,t}\mu_{a,t-1} + \hat{S}_{b,t-1}\mu_{b,t-1}). 
    \end{split}
  \end{equation}
...
\begin{align}
  \overline{S}_{Tr}^{-1} &=\frac{1}{T}\sum_{t=1}^TS_{u,t}^{-1} +\frac{1}{T}S_{Tr}^{-1}\left(\sum_{t=1}^T\hat{S}_{a,t}
   ...
   [S_{s,t}^{-1}+(\mu_{s,t}-\mu_{a,t-1})(\mu_{s,t}-\mu_{a,t-1})^T]\hat{S}_{a,t}\right)S_{Tr}^{-1}.\label{Sigma_Tr:2}
  \end{align}

Finally, we can make the following simplification by using \eqref{gamma_su}.
\begin{align*}
  \mu_{s,t} - \mu_{a,t-1} &= (S_{s,t}^{-1}\hat{S}_{a,t} - I)\mu_{a,t-1} + S_{s,t}^{-1}\hat{S}_{b,t-1}\mu_{b,t-1}\\
  &= (S_{s,t}^{-1}\hat{S}_{a,t} - S_{s,t}^{-1}S_{s,t})\mu_{a,t-1} + S_{s,t}^{-1}\hat{S}_{b,t-1}\mu_{b,t-1}\\
  &= -S_{s,t}^{-1}\hat{S}_{b,t-1}\mu_{a,t-1} + S_{s,t}^{-1}\hat{S}_{b,t-1}\mu_{b,t-1}\\
  &= S_{s,t}^{-1}\hat{S}_{b,t-1}(\mu_{b,t-1}-\mu_{a,t-1}).
\end{align*}
\end{document}
However, before substituting this into \eqref{Sigma_Tr:2}, there are some cancellations we can make.
\begin{align*}
  S_{s,t}^{-1}\hat{S}_{b,t} = S_{a,t-1})^{-1}S_{s,t}(S_{tr}^{-1}*S_{a,t-1}*\hat{S}_{b,t}),
