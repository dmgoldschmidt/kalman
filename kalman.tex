...
which we denote by $\hat{\gamma}_t(s,u)$. The joint probability density of all the data and the two given states
  ...
  \begin{align}
    ...
    &\propto \N(u;\mu_{u,t},S_{u,t})\N(s;\mu_{a,t-1},S_{Tr}*S_{a,t-1})\N(s;\mu_{b,t-1},M^TS_{Ob}M+S_{b,t})\notag\\
  ...
  \begin{equation}\label{gamma_su}
    \begin{split}
      ...
      S_{s,t} &:= S_{Tr}*S_{a,t-1} + \hat{S}_{b,t-1},\quad\text{and}\\
      \mu_{s,t} &:= S_{s,t}^{-1}(S_{Tr}*S_{a,t-1}\mu_{a,t-1} + \hat{S}_{b,t-1}\mu_{b,t-1}). 
    \end{split}
  \end{equation}
...
\end{align*}
Multiplying by $S_{Ob}^{-1}$, we find that
...
$$
  \int_{-\infty}^{\infty}\N(u;\mu_{u,t},S_{u,t})(u-s)(u-s)^Tdu = S_{u,t} + (s-\mu_{u,t})(s-\mu_{u,t})^T.
$$
Substituting this result into \eqref{Sigma_Tr:0}, we get
\begin{equation}\label{Sigma_Tr:1}
  \overline{S}_{Tr}^{-1} =  \frac{1}{T}\sum_{t=1}^T\left(S_{u,t} +\int_{-\infty}^{\infty}\N(s;\mu_{s,t},S_{s,t})
...
\begin{align}
  \overline{S}_{Tr}^{-1} &= \frac{1}{T}\sum_{t=1}^TS_{u,t} + \frac{1}{T}\sum_{t=1}^T
    \int_{-\infty}^{\infty}\N(s;\mu_{s,t},S_{s,t})S_{u,t}^{-1}S_{a,t-1}(s-\mu_{a,t-1})
    (s-\mu_{a,t-1})^TS_{a,t-1}S_{u,t}^{-1}ds\notag\\
    &=\frac{1}{T}\sum_{t=1}^TS_{u,t} +\frac{1}{T}\sum_{t=1}^TS_{u,t}^{-1}S_{a,t-1}
    \left[\int_{-\infty}^{\infty}\N(s;\mu_{s,t},S_{s,t})(s-\mu_{a,t-1})(s-\mu_{a,t-1})^Tds\right]S_{a,t-1}S_{u,t}^{-1}\notag\\
   &=\frac{1}{T}\sum_{t=1}^TS_{u,t}+\frac{1}{T}\sum_{t=1}^TS_{u,t}^{-1}S_{a,t-1}
   [S_{s,t}+(\mu_{s,t}-\mu_{a,t-1})(\mu_{s,t}-\mu_{a,t-1})^T]S_{a,t-1}S_{u,t}^{-1}.\label{Sigma_Tr:2}
  \end{align}

Finally, we can make the following simplification by using \eqref{gamma_su}.
\begin{align*}
  \mu_{s,t} - \mu_{a,t-1} &= (S_{s,t}^{-1}S_{Tr}*S_{a,t-1} - I)\mu_{a,t-1} + S_{s,t}^{-1}\hat{S}_{b,t-1}\mu_{b,t-1}\\
  &= (S_{s,t}^{-1}S_{Tr}*S_{a,t-1} - S_{s,t}^{-1}S_{s,t})\mu_{a,t-1} + S_{s,t}^{-1}\hat{S}_{b,t-1}\mu_{b,t-1}\\
  &= -S_{s,t}^{-1}\hat{S}_{b,t-1}\mu_{a,t-1} + S_{s,t}^{-1}\hat{S}_{b,t-1}\mu_{b,t-1}\\
  &= S_{s,t}^{-1}\hat{S}_{b,t-1}(\mu_{b,t-1}-\mu_{a,t-1}).
\end{align*}
and substituting this into \eqref{Sigma_Tr:2} we have
\begin{equation}\label{Sigma_Tr:3}
\overline{S}_{Tr} = \frac{1}{T}\sum_{t=1}^TS_{u,t}+\frac{1}{T}\sum_{t=1}^TS_{s,t}^{-1}\hat{S}_{b,t-1}
   [S_{s,t}+(\mu_{b,t-1}-\mu_{a,t-1})(\mu_{b,t-1}-\mu_{a,t-1})^T]\hat{S}_{b,t-1}S_{s,t}^{-1}.
\end{equation}  
\end{document}
