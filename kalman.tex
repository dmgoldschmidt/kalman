...
Furthermore, if $A_1$ and $A_2$ are unimodular then 
...
\end{proof}
  Note that
  $$
  \int_{\R^{2n}}\N(s_{t-1};\mu_{t-1,t},S_{t-1,t})\N(s_t;\mu_{c,t}S_{c,t})ds_{t-1}ds_t = \int_{\R^n}\N(s_t;\mu_{c,t}S_{c,t})ds_t = 1 
  $$
  because the second factor does not depend on $s_{t-1}$, so \eqref{gamma_hat} is a bona-fide probability density and no
...
\label{E1}\\
E_2 :&= \int_{\R^{Tn}}P(X,S\mid\theta)\left\{\sum_{t=1}^T\log|\overline{S}_T| - Q(s_t;\overline{T}s_{t-1},
\overline{S}_T)\right\}dS,
\label{E2}
\end{align}
...
E_1 &= T\log|\overline{S}_M| - \sum_{t=1}^T\int_{\R^n}\gamma_t(s_t)Q(x_t;Ms_t,\overline{S}_M)ds_t.
\end{align*}
...
$$
E_2 = T\log|\overline{S}_T| - \sum_{t=1}^T\int_{\R^{2n}}\hat{\gamma}_t(s_{t-1},s_t)Q(s_t;Ts_{t-1},\overline{S}_T)ds_{t-1}ds_t,
$$
...
\begin{align*}
  ...
  - \sum_{t=1}^T\int_{\R^n}\gamma_t(s_t)\frac{\partial{Q(x_t;\overline{M}s_t,\overline{S}_M)}}{\partial{\overline{S}_M}}ds_t \\
...
\begin{equation}\label{S_M}
  \begin{split}
    \overline{S}_M^{-1} &= \frac{1}{T}\sum_{t=1}^T\int_{\R^n}\gamma_t(s_t)(x_tx_t^T - x_ts_t^T\overline{M}^T
...
Next, we optimize $E_1$ with respect to $\overline{M}$, optionally subject to the constraints that the diagonal elements
of $S' := \overline{M}^TS_M\overline{M}$ are all equal to $1$.  To do this, we make a unitary change of
state variable $s_t := U_ts'_t$, where $U_t$ will be determined below. Thus
$$
\gamma_t(s_t') = \N(s_t';U_t^T\mu_{c,t},U_t^TS_{c,t}U_t).
$$
Then we differentiate
$$
E_1 + \sum_{j=1}^n\lambda_j\left(1 - \sum_{i=1}^m\bar{m}_{ij}S'_{ik}\bar{m}_{kj}\right)
$$
with respect to $\overline{M}$ using \eqref{dM}, where the $\lambda_j$ are Lagrange multipliers.
Put $\Lambda = diag\{\lambda_1,\dots,\lambda_n\}$.  Note that the constraint equations do not depend on $t$.
Then setting the derivative to zero, we must solve
\begin{align*}
%  - \sum_{t=1}^T\int_{\R^n}\gamma_t(s')\frac{\partial{Q(x_t;\overline{M}Us',\overline{S}_M)}}{\partial{\overline{M}}}ds
  0  &= 2\sum_{t=1}^T\int_{\R^n}\gamma_t(s_t')(\overline{S}_Mx_ts_t'^TU_t^T - \overline{S}_M\overline{M}U_ts_t's_t'^TU_t^T)ds' +
  2S_M\overline{M}\Lambda\\
&= 2\sum_{t=1}^T(\overline{S}_Mx_t\mu_{c,t}^TU - \overline{S}_M\overline{M}U^T(S_{c,t}^{-1} + \mu_{c,t}\mu_{c,t}^T)U +
2S_M\overline{M}\Lambda,
\end{align*}
Multiplying through by $\frac{1}{2}S_M^{-1}$, we find that in the unconstrained case,
...
$$
  \Gamma_1U = \overline{M}(U^T\Gamma_2U + \Lambda),
$$
and then set the $\lambda_j$ appropriately to satisfy the constraints. We see that if we choose $U$ to diagonalize
$\Gamma_2$ and set $\Lambda^* := (U^T\Gamma_2U + \Lambda)^{-1}$, then
\begin{equation}\label{M_bar:1}
  \overline{M} = \Gamma_1U\Lambda^*,
\end{equation}
where 
$$
\lambda_j^* = \sqrt{(U^T\Gamma_1^TS_M\Gamma_1U)_{jj}}\quad(1\le j\le n).
$$

Note that in the unconstrained case \eqref{M_bar} is independent of $S_M$, so to optimize $E_1$, we can first
solve for $\overline{M}$ and then use the result to solve for $\overline{S}_M$.  In the constrained case this
is unfortunately not true, so instead we could solve for both using previous values, or perhaps just solve alternately
for one and then the other on the next Baum-Welch iteration.
...
\begin{equation}\label{Sigma_Tr:0}
  ...
  - \sum_{t=1}^T\int_{\R^{2n}}\hat{\gamma}_t(s_{t-1},s_t)\frac{\partial{Q(s_t;Ts_{t-1},\overline{S}_T)}}{\partial\overline{S}_T}ds_{t-1}ds_t \\
  &= (2-\delta_{ij})T\overline{S}_T^{-1} - (2-\delta{ij}) \sum_{t=1}^T\int_{\R^{2n}}\N(s_{t-1};\mu_{t-1,t},S_{t-1,t})\gamma_t(s_t)(s_{t-1}-s_t)
  (s_{t-1}-s_t)^Tds_{t-1}ds_t.
  \end{split}
\end{equation}
...
$$
\int_{-\infty}^{\infty}\N(s_{t-1};\mu_{t-1,t},S_{t-1,t})(s_{t-1}-s_t)(s_{t-1}-s_t)^Tds_{t-1} = S_{t-1,t}^{-1} +
(s_t-\mu_{t-1,t})(s_t-\mu_{t-1,t})^T.
$$
Substituting this result into \eqref{Sigma_Tr:0}, we get
\begin{equation}\label{Sigma_Tr:1}
  \overline{S}_T^{-1} =  \frac{1}{T}\sum_{t=1}^T\left(S_{t-1,t}^{-1} +\int_{-\infty}^{\infty}\gamma_t(s_t)
  (s_t-\mu_{t-1,t})(s_t-\mu_{t_1,t})^Tds_t\right).
\end{equation}
...
\begin{equation}\label{s-mu_1}
  \begin{split}
    s-\mu_{t-1,t} &= (I - S_{t-1,t}^{-1}S_T)s_t - S_{t-1,t}^{-1}S_{a,t-1}\mu_{a,t-1}, \\
    &= (S_{t-1,t}^{-1}S_{t-1,t} - S_{t-1,t}^{-1}S_T)s_t - S_{t-1,t}^{-1}S_{a,t-1}\mu_{a,t-1}, \\
    &= S_{t-1,t}^{-1}(S_{t-1,t} - S_T)s_t - S_{t-1,t}^{-1}S_{a,t-1}\mu_{a,t-1}, \\
    &= S_{t-1,t}^{-1}S_{a,t-1}(s_t - \mu_{a,t-1}),\\
  \end{split}
\end{equation}


Fortunately, neither $\mu_{a,t-1},S_{a,t-1},$ nor $S_{t-1,t}$ depends on $s_t$,
so using \eqref{gamma_hat}, \eqref{Sigma_Tr:1} becomes
\begin{equation}\label{S_Tr:2}
  \begin{split}
  \overline{S}_T^{-1} &=\frac{1}{T}\sum_{t=1}^T\left(S_{t-1,t}^{-1} +S_{t-1,t}^{-1}S_{a,t-1}
  \left[\int_{-\infty}^{\infty}\gamma_t(s_t)(s_t-\mu_{a,t-1})(s_t-\mu_{a,t-1})^Tds_t\right]S_{a,t-1}S_{t-1,t}^{-1}\right)\\
   &=\frac{1}{T}\sum_{t=1}^TS_{t-1,t}^{-1}+S_{t-1,t}^{-1}S_{a,t-1}
   [S_{c,t}^{-1}+(\mu_{c,t}-\mu_{a,t-1})(\mu_{c,t}-\mu_{a,t-1})^T]S_{a,t-1}S_{t-1,t}^{-1}.
  \end{split}
\end{equation}

Finally, we complete our optimization of $E_2$ by differentiating with respect to $T$ and imposing the constraint
$\det(T) = 1$.
